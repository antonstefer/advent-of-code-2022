...
fn ranges_overlap(range_one: (usize, usize), range_two: (usize, usize)) -> bool {
    if range_one.0 <= range_two.0 && range_one.1 >= range_two.0 {
        return true;
    }
    if range_two.0 <= range_one.0 && range_two.1 >= range_one.0 {
        return true;
    }
    return false;
}
